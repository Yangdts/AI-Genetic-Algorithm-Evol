...
class Genome():
    ...
    #Motor function crossover.        
    # def crossover_motor_functions(parent1_genome, parent2_genome):
    #     child_genome = copy.deepcopy(parent1_genome)
        
    #     for gene_index, (gene1, gene2) in enumerate(zip(parent1_genome, parent2_genome)):
    #         for i, (allele1, allele2) in enumerate(zip(gene1, gene2)):
    #             if i == 0:  # Waveform type (PULSE or SINE)
    #                 child_genome[gene_index][i] = random.choice([allele1, allele2])
    #             elif i == 1 or i == 2:  # Amplitude and Frequency
    #                 child_genome[gene_index][i] = (allele1 + allele2) / 2.0
        
    #     return child_genome

    # def mutate_motor(genome, rate, amount):
